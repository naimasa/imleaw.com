...
|:---:|:---|:---|:---|:--:|
| 1 | 20260905-070709-wordpress-static-migration | 2026-09-05 | 初版（Astro による静的サイト移行アーキテクチャ設計）。実サイトへのネットワークアクセスが得られず、記事数・カテゴリ・固定ページ・画像量を **（要 Phase 0 確認）** として保留 | ⏸ |
| **2** | 20260905-070709-wordpress-static-migration | **2026-09-05** | **実サイト棚卸し（WP REST API）を完了し、Rev 1 の未確定事項をすべて実測値で解決。** 主な確定・変更点: ①記事 431 本 / 固定ページ 11 枚 / カテゴリ 5 件 / メディア 565 点を確定し、全 **（要 Phase 0 確認）** 表記を削除 ②トップ `/` は**静的フロントページ**、記事一覧は `/blogs/` であることが判明し、ルーティングを全面改訂（§6.4） ③スラッグが**パーセントエンコード日本語**であることを確認し、「保存形式＝エンコード済み／出力形式＝デコード済みディレクトリ名」の二表現規約を新設（§6.3、本改訂の最重要事項） ④タグ taxonomy 不使用を確認し `/tag/` ルートを廃止 ⑤`-scaled` 派生の削除方針を撤回（WP の `full` URL 実体のため保持必須・§7.1） ⑥`@astrojs/sitemap` を自前生成に差し替え（エンコード形式のバイト一致担保・§6.6） ⑦画像 565 点の実サイズを踏まえ N-07 ビルド時間目標を改訂（§4） ⑧Decap CMS / Formspree / MapEmbed / Instagram 静的グリッド / Astro 5.x SSG を**確定**（§8・§11） ⑨検証スクリプトの期待値を実データ（431 / 11 / 5 / 565 / 533 URL）に固定（§9） | ⏸ |
